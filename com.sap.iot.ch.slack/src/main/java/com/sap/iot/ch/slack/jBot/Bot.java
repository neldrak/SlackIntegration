package com.sap.iot.ch.slack.jBot;
...
/**
 * Base class for making Slack Bots. Any class extending this will get all
 * powers of a Slack Bot.
 *
 * @author ramswaroop
...
public abstract class Bot {
	...
	/**
	 * A Map of all methods annotated with {@link Controller} where key is the
	 * {@link EventType#name()} and value is a list of {@link MethodWrapper}.
	 * NOTE: It does not contain methods which are part of any conversation.
	 */
	private final Map<String, List<MethodWrapper>> eventToMethodsMap = new HashMap<>();
	/**
	 * A Map of all methods annotated with {@link Controller} where key is the
	 * {@link Method#getName()} and value is the respective
	 * {@link MethodWrapper}.
	 */
	...
	/**
	 * A List of Queues with each Queue holding all methods for a particular
	 * conversation. Methods can be chained into a conversation by
	...
	/**
	 * Class extending this must implement this as it's required to make the
	 * initial RTM.start() call.
	 *
	 * @return
	 */
	...
	/**
	 * An instance of the Bot is required by the {@link BotWebSocketHandler}
	 * class.
	 *
	 * @return
	 */
	...
	/**
	 * Invoked after a successful web socket connection is established. You can
	...
	/**
	 * Invoked after the web socket connection is closed. You can override this
		...
		try {
			...
			if (event.getType() != null) {
				if (event.getType().equalsIgnoreCase(EventType.IM_OPEN.name())
					...
					} else if (slackService.getDmChannels().contains(event.getChannelId())) { // direct
																								// message
						event.setType(EventType.DIRECT_MESSAGE.name());
					}
				}
			} else { // slack does not send any TYPE for acknowledgement
						// messages
	...
	/**
	 * Form a Queue with all the methods responsible for a particular
	 * conversation.
	 *
	 * @param queue
	 * @param methodName
	...
	/**
	 * Invoke the methods with matching {@link Controller#events()} and
	 * {@link Controller#pattern()} in events received from Slack.
	 *
	 * @param session
	 * @param event
	 */
	...
	/**
	 * Search for a method whose {@link Controller#pattern()} match with the
	 * {@link Event#text} in events received from Slack and also filter out the
	 * methods whose {@link Controller#pattern()} do not match with slack
	 * message received ({@link Event#text}) for cases where there are no
	 * matched methods.
	 *
	 * @param event
	 * @param methodWrappers
	 * @return the MethodWrapper whose method pattern match with that of the
	 *         slack message received, {@code null} if no such method is found.
	 */
	private MethodWrapper getMethodWithMatchingPatternAndFilterUnmatchedMethods(Event event,
			...
			while (listIterator.hasNext()) {
				...
				if (!StringUtils.isEmpty(pattern) && !StringUtils.isEmpty(text)) {
					...
					if (m.find()) {
						...
						listIterator.remove(); // remove methods from the
												// original list whose pattern
												// do not match
					}
				}
			}
	...
	/**
	 * Entry point where the web socket connection starts and after which your
	 * bot becomes live.
	 */
	@PostConstruct
